...
export const toolSchemas = {
  crypto_prices: {
    ...
    service: "price",
    transform: (input: { symbols: string[] }) => ({ symbols: input.symbols }),
  },
  market_sentiment: {
  ...
  funding_rate: {
    ...
    service: "funding",
    transform: (input: { symbol: string }) => ({ symbol: input.symbol }),
  },
  technical_indicators: {
    ...
    service: "indicators",
    transform: (input: { symbol: string; interval?: string }) => ({ symbol: input.symbol, interval: input.interval || "1h" }),
  },
  market_overview: {
  ...
  defi_yields: {
    ...
    service: "yields",
    transform: (input: any) => {
      const out: Record<string, any> = { limit: input.limit || 10 };
      if (input.chain) out.chain = input.chain;
      if (input.project) out.project = input.project;
      if (input.symbol) out.symbol = input.symbol;
      if (input.minApy != null) out.minApy = input.minApy;
      if (input.minTvl != null) out.minTvl = input.minTvl;
      if (input.stablecoins != null) out.stablecoins = input.stablecoins;
      return out;
    },
  },
  gas_price: {
    ...
    service: "gas",
    transform: (input: { chain: string }) => ({ chain: input.chain }),
  },
  yield_pool_metrics: {
    ...
    transform: (input: any) => {
      ...
      if (input.protocols) out.protocols = input.protocols;
      if (input.pool_ids) out.pool_ids = input.pool_ids;
      return out;
    },
  },
  fresh_markets: {
    ...
    transform: (input: any) => {
      const out: Record<string, any> = {};
      if (input.chain) out.chain = input.chain;
      if (input.window_minutes) out.window_minutes = input.window_minutes;
      if (input.limit) out.limit = input.limit;
      return out;
    },
  },
  ...
  get_price: {
    ...
    service: "price",
    transform: (input: any) => ({ symbols: [input.token] }), // map token -> symbols array
  },
  ...
  get_funding_rates: {
    ...
    service: "funding",
    transform: (input: any) => ({ symbol: input.symbol || "BTC" }),
  },
  get_technical_indicators: {
    ...
    service: "indicators",
    transform: (input: any) => ({ symbol: input.symbol, interval: input.timeframe || "1h" }),
  },
  get_defi_yields: {
    ...
    service: "yields",
    transform: (input: any) => {
      const out: Record<string, any> = { limit: 10 };
      if (input.chain) out.chain = input.chain;
      if (input.protocol) out.project = input.protocol;
      if (input.min_tvl) out.minTvl = input.min_tvl;
      if (input.min_apy) out.minApy = input.min_apy;
      return out;
    },
  },
  get_gas_price: {
    ...
    service: "gas",
    transform: (input: any) => ({ chain: input.chain || "ethereum" }),
  },
  get_multi_chain_gas: {
    ...
    service: "gas_multi",
    transform: (input: any) => ({ chains: input.chains || ["ethereum", "base", "arbitrum", "optimism", "polygon", "bsc", "avalanche"] }),
  },
  get_pool_metrics: {
    ...
    transform: (input: any) => {
      const out: Record<string, any> = {};
      if (input.chain) out.chain = input.chain;
      if (input.dex) out.dex = input.dex;
      if (input.min_apy) out.minApy = input.min_apy;
      if (input.min_tvl) out.minTvl = input.min_tvl;
      return out;
    },
  ...
  get_pool_alerts: {
    ...
    transform: (input: any) => {
      const out: Record<string, any> = {};
      if (input.chain) out.chain = input.chain;
      if (input.alert_type) out.alert_type = input.alert_type;
      return out;
    },
  },
  scan_new_pairs: {
    ...
    transform: (input: any) => {
      ...
      if (input.chain) out.chain = input.chain;
      if (input.dex) out.dex = input.dex;
      if (input.min_liquidity) out.min_liquidity = input.min_liquidity;
      return out;
    },
  },
} as const;
...
export const stdioToHttpMap: Record<string, string> = {
...
export const httpToStdioMap: Record<string, string> = {
